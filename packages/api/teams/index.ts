...
export async function getTeam(name: string) {
  try {
    const { data, error } = await supabase
      .from('teams')
      .select(
        'id, name, slug, contract_address, prefix, creator_address, bootstrap_address, active, tx_id, bootstrap_tx_id, activation_tx_id, config, extensions (contract_address, tx_id, config, extension_types (name))',
      )
      .eq('slug', name);
...
export async function generateContractName(organization: any) {
  try {
    const { data: Proposals, error } = await supabase
      .from('proposals')
      .select('contract_address, teams!inner(id, name, prefix)')
      .eq('teams.id', organization?.id);
    if (error) throw error;
...
export async function getContractsToDeploy(
  ...
  try {
    const { data, error } = await supabase
      .from('proposals')
      .select(
        'id, contract_address, type, tx_id, submitted, disabled, proposed_by, teams!inner(id, name, prefix)',
      )
      .eq('teams.id', organizationId)
...
export async function getUserteams(address: string | undefined) {
  try {
    const { data: teams, error } = await supabase
      .from('users_teams')
      .select(
        'user_address, team_id, team:teams!inner(id, name, slug, contract_address)',
      )
